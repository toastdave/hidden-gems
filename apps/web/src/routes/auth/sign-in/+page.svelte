...
<svelte:head>
	<title>Sign In | Hidden Gems</title>
</svelte:head>
...
<section class="mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl items-center px-6 py-10 sm:px-8 lg:px-10">
	<div class="grid w-full gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
		<div class="space-y-5 rounded-[2rem] border border-white/70 bg-white/70 p-8 shadow-[0_32px_120px_-48px_rgba(19,34,38,0.45)] backdrop-blur">
			<p class="font-display text-sm uppercase tracking-[0.3em] text-coral-500">Welcome back</p>
			<h1 class="font-display text-4xl leading-none text-ink-950 sm:text-5xl">Sign in to keep track of your finds.</h1>
			<p class="max-w-xl text-base leading-8 text-ink-700">
				Save favorites, follow hosts, and get ready for host tools and weekend alerts.
			</p>
		...
		<div class="rounded-[2rem] border border-ink-950/10 bg-white/85 p-8 shadow-[0_24px_90px_-54px_rgba(19,34,38,0.55)] backdrop-blur">
			<form class="space-y-5" onsubmit={signInWithEmail}>
				<div class="space-y-2">
					...
					<input
						bind:value={email}
						class="w-full rounded-2xl border border-ink-950/10 bg-white px-4 py-3 text-sm text-ink-950 outline-none transition focus:border-coral-500"
						id="email"
						required
						type="email"
					/>
			...
			<p class="mt-6 text-sm text-ink-700">
				Need an account?
				<a class="font-semibold text-ink-950 underline decoration-coral-500 underline-offset-4" href={`/auth/sign-up?redirectTo=${encodeURIComponent(redirectTo)}`}>
					Create one
				</a>
			</p>
		</div>
