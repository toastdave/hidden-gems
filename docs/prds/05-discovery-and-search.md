...
- Completed: discovery-first homepage, DB-backed published listings, map/list sync, sharable date/type/radius filtering, listing detail pages, host detail pages, related listings, product-facing discovery copy, MapTiler-backed location geocoding, and browser geolocation fallback messaging.
- In progress: media thumbnails across discovery cards, broader SEO metadata, stronger performance/code-splitting polish, and richer location autocomplete UX.
