...
Status: in progress. Favorites on discovery cards and listing detail pages, host follows on host and listing surfaces, saved account views for listings and hosts, first-pass saved searches from discovery into account, account-level alert preferences, and manual-run in-app saved-search delivery are now live. Email delivery automation remains the next retention layer.
